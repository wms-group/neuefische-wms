...
const SearchableSelect: FC<SearchableSelectProps> = (props) => {
    ...
    return (
        <Select
            ...
            classNames={{
                ...props.classNames,
                container: ({isFocused}) => cn('h-[42px] rounded-lg border border-secondary bg-white/95 m-0 p-0', isFocused && "outline-none ring-2 ring-blue-500", props.className, props.classNames?.container ?? ''),
                control: () => cn('border-none px-3 py-1.5', props.classNames?.control ?? ''),
                menu: () => cn('text-gray-900 rounded mt-1 bg-gray-100 m-0 p-0 min-w-fit z-1 ring ring-blue-500', props.classNames?.menu ?? ''),
                groupHeading: () => cn('text-gray-400 bg-gray-100 text-xs border-b border-gray-200 px-4 pt-1 pb-0.5 z-100 first-of-type:rounded-t', props.classNames?.groupHeading ?? ''),
                option: ({isFocused, isSelected}) => cn("text-gray-600 whitespace-nowrap px-2 py-1 m-0", isFocused && 'bg-white/45 text-gray-400', isSelected && 'bg-white/80 text-gray-700', props.classNames?.option ?? ''),
            }}
